...
const PresentationsPage = () => {
    ...
    const handleCreatePresentation = async () => {
        if (!title || !file) {
            setMsg("  ⚠️   Please provide title and presentation file.");
            return;
        }

        if (isStudentSelectorLoading) {
            setMsg("  ⚠️   Please wait for student data to load.");
            return;
        }

        if (assignedStudents.length === 0) {
            setMsg("  ⚠️   Please select at least one student to assign the presentation, or ensure students are available.");
            return;
        }
    ...
    const handleGradeSubmission = async (submission) => {
        ...
        if (marksValue !== null && (isNaN(marksValue) || marksValue < 0 || marksValue > 100)) {
            setGradingMsg("  ⚠️   Marks must be between 0 and 100.");
            return;
        }
                                ...
                                if (isTeacher || isAssigned) {
                                    return (
                                        <div key={presentation._id} className="assignment-card">
                                            ...
                                            <p>
                                                Uploader: {presentation.uploader?.name || 'Teacher'} |
                                            ...
                                            <div className="actions">
                                                ...
                                                {isTeacher ? (
                                                    ...
                                                    <>
                                                        ...
                                                        >
                                                            View Submissions ({allSubmissions.filter(s => s.presentationId === presentation._id).length})
                                                        </button>
                                                    </>
                                                ) : (
                        ...
                        <div className="popup">
                            ...
                            {submissionTarget.fileId ? (
                                <div style={{ marginTop: '15px' }}>
                                    ...
                                    >
                                        Download Question ({submissionTarget.originalName})
                                    </button>
                                            </div>
                            ...
                            {submissions.length > 0 ? (
                                <>
                                    ...
                                    <div className="submission-status-card">
                                        <p>File: {submissions[0].originalName}</p>
                                        <p>
                                            Status:{" "}
                                ...
                                <p>No submission found. Upload your file below.</p>
                            )}

                            <hr />
                            <h4>{submissions.length > 0 ? "Replace File" : "Upload File"}</h4>
                            <input
                                type="file"
                                onChange={(e) => setFile(e.target.files[0])}
                            />
