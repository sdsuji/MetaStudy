...
    // Convert backend's ISO date string to YYYY-MM-DD for input type="date"
    const formattedDate = assignment.dueDate ? new Date(assignment.dueDate).toISOString().split('T')[0] : '';
    setEditDueDate(formattedDate);
    setUploadMsg(""); // Clear messages
    setShowEditPopup(true);
  };
  
  // NEW: Handle assignment update submission
...
      setUploadMsg("⚠️ Please provide title, file, and due date.");
      return;
    }
    try {
      setUploading(true);
...
      // Check for specific backend errors if needed, otherwise use generic message
...
                        </>
                      ) : (
                        // Student button - always enabled to view marks/feedback
                        <button
                          onClick={() => {
                            setSubmissionTarget(a);
                            setShowSubmissionPopup(true);
                            fetchSubmissions(a._id);
                            setSubmissionMsg(""); // clear previous messages
                          }}
                        >
                          Submit
                        </button>
...
                    <p className="past-due">⚠️ Submission closed. Past due date.</p>
                  ) : (
                    <>
...
                <div className="submissions-list-student"> {/* Added student list class for consistency */}
